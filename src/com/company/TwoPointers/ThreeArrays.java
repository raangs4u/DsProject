...
public class ThreeArrays {
    ...
    public static int minimize(final List<Integer> a, final List<Integer> b, final List<Integer> c) {
        ...
        int z=0;
        while(x<m && y<n && z<l) {
            int sa = max(a.get(x), b.get(y), c.get(z)) - min(a.get(x), b.get(y), c.get(z));
            if (sa < min) {
                min = sa;
            }
            if(a.get(x) <=b.get(y) && a.get(x)<= c.get(z)) {
                x++;
            } else if (b.get(y)<=a.get(x) && b.get(y)<=c.get(z)) {
                y++;
            } else {
                z++;
            }
        ...
        for(int i=0;i<m;i++){
            ...
            int res = max(Math.abs(a.get(i)-b.get(j)),Math.abs(c.get(k)-b.get(j)),Math.abs(a.get(i)-c.get(k)));
            if(min1>res){
                min1 = res;
            }
        }
